...
var KTApp = function () {
    ...
    var handleAjaxSetup = function () {
        // CSRF Token
        var csrftoken = KTCookie.getCookie('csrftoken');
        $.ajaxSetup({
            cache: false,
            abortOnRetry: true,
        ...
        $(document).ajaxStart(function () {
            ...
            if (textStatus.status === 403)
                ...
                KTApp.alert('An ajax error occured');
            }
    ...
    return {
        ...
        block: function (target, options) {
            ...
            if (options.message && options.message.length > 0) {
                ...
                options.width = KTUtil.actualWidth(elDiv) + 10;
                KTUtil.remove(elDiv);
                if (target === 'body') {
        ...
        makeAjaxForm: function (form_elem) {
            ...
            frm.submit(function (e) {
                e.preventDefault();
                if (!$(this).valid()) {
                    KTApp.notify('Zorunlu alanları doldurunuz!!!', 'error');
                    return;
                }
            ...
            KTApp.validator('#form');
        },
        reloadDataTables: function () {
            $.each(KTApp.datatable, function (item) {
                KTApp.datatable[item].getDataTable().ajax.reload();
            });
        },
        ...
        load: (function () {
            var _load = function (tag) {
                return function (url) {
                    // Get system version
                    var version = KTCookie.getCookie('app-version');
        ...
        loadFiles: function (srcs, callback) {
            if ($.isArray(srcs) && srcs.length) {
                ...
                srcs.forEach(function (src) {
                    ...
                    switch (type) {
                        case 'js':
                            loads.push(KTApp.load.js(src));
                            break;
                        case 'css':
                            loads.push(KTApp.load.css(src));
                            break;
                        case 'img':
                            loads.push(KTApp.load.img(src));
                            break;
                    }
                });
        ...
        fullHeight: function (className, callback) {
            App._fullHeight(className, callback);
            $(window).resize(function () {
                App._fullHeight(className, callback);
            });
        },
        _fullHeight: function (className, callback) {
                ...
                $('<div/>').addClass('modal-dialog').append(
                    $('<div/>').addClass('modal-content').append(
                        $('<div/>').addClass('modal-header').append(
                            ...
                            }).addClass('close').html('<i aria-hidden="true" class="ki ki-close"></i>'),
                        ),
        ...
        dialogForm: function (title, url, options) {
            ...
            $('<div/>').addClass('modal-body').load(url, function () {
                ...
                if (options && options['close_callback']) {
                    modal.on('hidden.bs.modal', function () {
                        eval(options['close_callback'] + '();')
                    });
                }
                modal.find('.modal-content').append(
                    $('<form/>', {
                        ...
                        $.ajax({
                            ...
                            if (result.success) {
                                if (result.info) {
                                    KTApp.alert(result.message, 'info', 'Bilgilendirme');
                                } else {
                                    KTApp.notify(result.message);
                                }
                                $this.closest('.modal').modal('hide');
                                KTApp.reloadDataTables();
                                if (options) {
                                    if (options.hasOwnProperty('callback')) {
                                        eval(options['callback'] + '(result);');
                                    }
                                    if (options.hasOwnProperty('redirect')) {
                                        window.location.href = result[options['redirect']];
                                    }
                                }
                ...
                KTApp.validator(modal.find('.modal-content').find('form'));
        ...
        dialogInput: function (title, onConfirm, options) {
            var submitText = (options && options['submit-text']) ? options['submit-text'] : 'Tamam';
            var content = $('<div/>').addClass('modal-input').append(
                $('<textarea/>', {'rows': 20,  'class':'form-control', 'id':'modal-input-result'})
            );
            var buttons = [
                $('<button/>', {
                    'type': 'button',
                ...
                $('<button/>', {
                    'type': 'button',
                    'data-dismiss': 'modal'
                }).addClass('btn btn-primary').text(submitText).click(function () {
                    onConfirm($('#modal-input-result').val().trim());
                })
            ];
            var modal = KTApp.modal(title, content, buttons);
            if (options && options['large']) {
                modal.find('.modal-dialog').addClass('modal-lg');
            } else if (options && options['small']) {
                modal.find('.modal-dialog').addClass('modal-sm');
        ...
        openWindow: function (url, action) {
            ...
            if (action === 'print') {
                ...
                popup = window.open(url, action, 'noopener');
            }
        },
        openLocation: function (provider, lat, lon) {
            ...
            switch (provider) {
                case 1: //google
                    url = App.stringFormat('https://www.google.com/maps/search/?api=1&query={0},{1}', lat, lon);
                    break;
                case 2: //yandex
        ...
        validator: function (form) {
            return $(form).validate({
                ...
                errorPlacement: function (error, element) {
                    ...
                    else if (element.parents('div').hasClass('bootstrap-select') ) {
                        error.appendTo(element.parent().parent());
                    }
...
KTUtil.ready(function () {
    ...
    // Init Chat KTApp.(quick modal chat)
    //KTLayoutChat.init();
...
String.prototype.toUpperTR = function () {
    return this.replace(/i/g, "\u0130").replace(/^\s+|\s+$/g, "").toUpperCase();
};

function toUpperTR(val) {
...
function toFloatEn(val) {
    return parseFloat(val.replace(/[^0-9.,]/, '').replace('.', '').replace(',', '.'));
}

function toFloatTr(val) {
    return val.replace(/[^0-9.,]/, '').replace(',', '').replace('.', ',');
}

function toDecimalFormat(val) {
    return parseFloat(val).format_decimal(2, ',', '.', '');
}

String.prototype.replaceAllStrings = function (search, replacement) {
    var target = this;
